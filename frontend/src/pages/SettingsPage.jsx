...
function SettingsPage({ settings, accounts, busy, saveSettings, runDataMigration, refreshAll, refreshing, logout }) {
  ...
  return (
    <section className="space-y-5 sm:space-y-6">
      ...
      {/* Account Logout */}
      <div className="glass-panel-dark overflow-hidden rounded-2xl border-rose-200/50">
        <div className="border-b border-rose-300/80 bg-gradient-to-r from-rose-100/70 to-orange-100/70 px-4 py-4 sm:px-6 sm:py-5">
          <h2 className="font-heading text-lg font-semibold text-slate-900 sm:text-xl">🚪 Account Session</h2>
          <p className="mt-1 text-xs text-slate-600 sm:text-sm">Safely sign out from the automation system on this device.</p>
        </div>
        <div className="p-4 sm:p-6">
          <div className="rounded-xl border border-rose-200 bg-rose-50/50 p-4 sm:p-5">
            <h3 className="text-sm font-semibold text-slate-800">Logout</h3>
            <p className="mt-1 text-xs text-slate-500">
              This will clear your local session and return you to the login screen. 
              Running campaigns will not be affected.
            </p>
            <div className="mt-4">
              <button 
                onClick={logout}
                className="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white font-bold px-10 py-3.5 rounded-xl transition-all shadow-xl shadow-red-600/20 active:scale-[0.98]" 
              >
                Sign Out Now
              </button>
            </div>
